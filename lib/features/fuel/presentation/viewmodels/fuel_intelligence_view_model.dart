...
import 'package:fuel_tracker_app/features/fuel/intelligence/driving_behavior/driving_behavior_analyzer.dart';
import 'package:fuel_tracker_app/features/fuel/intelligence/driving_behavior/driving_behavior_models.dart';
import 'package:fuel_tracker_app/features/fuel/intelligence/prediction/fuel_prediction_engine.dart';
import 'package:fuel_tracker_app/features/fuel/intelligence/prediction/fuel_prediction_models.dart';
import 'package:fuel_tracker_app/features/fuel/intelligence/simulation/route_fuel_simulation_engine.dart';
import 'package:fuel_tracker_app/features/fuel/intelligence/telemetry/telemetry_sample.dart';
import 'package:fuel_tracker_app/features/fuel/intelligence/warnings/fuel_warning_models.dart';
import 'package:fuel_tracker_app/features/fuel/intelligence/warnings/warnings_engine.dart';
...
import 'package:fuel_tracker_app/features/fuel/data/services/elevation_service.dart';
...
import 'package:fuel_tracker_app/features/fuel/data/services/weather_service.dart';
...
class FuelIntelligenceViewModel extends ChangeNotifier {
  ...
  void start() {
    ...
    _bootTimeout?.cancel();
    _bootTimeout = Timer(const Duration(seconds: 6), () {
      if (!_isReady && _started) {
        _loadError ??= 'Tải dữ liệu quá lâu. Kiểm tra kết nối và thử lại.';
        _isReady = true;
        _prediction ??= _minimalPrediction();
        notifyListeners();
      }
    });
  ...
  Future<void> _step({bool forceStationsRefresh = false}) async {
    ...
    try {
      final behavior = _ingestBehaviorSample();
      _behaviorMetrics = behavior;

      _applyPrediction(behavior: behavior);
      _isReady = true;
      _loadError = null;
      _bootTimeout?.cancel();
      notifyListeners();
      ...
      await _enrichData(
        behavior: behavior,
        seq: seq,
        forceStationsRefresh: forceStationsRefresh,
      );
      if (seq != _workSeq || !_started) return;
      ...
      if (seq != _workSeq || !_started) return;
      _loadError = 'Không thể tải dữ liệu phân tích.';
      _isReady = true;
      _prediction ??= _minimalPrediction();
      notifyListeners();
    }
  ...
  DrivingBehaviorMetrics _ingestBehaviorSample() {
  ...
  Future<void> _enrichData({
    required DrivingBehaviorMetrics behavior,
    required int seq,
    required bool forceStationsRefresh,
  }) async {
    Future<void> guard(Future<void> future) {
      return future.timeout(const Duration(seconds: 14), onTimeout: () {});
    }

    final jobs = <Future<void>>[];
    if ((forceStationsRefresh || _nearbyStations.isEmpty) && !_stationsLoading) {
      jobs.add(guard(_loadNearbyStations(force: forceStationsRefresh)));
    }
    if (_weather == null && !_weatherLoading) {
      jobs.add(guard(_loadWeather()));
    }
    jobs.add(guard(_ensureRoute(behavior: behavior, seq: seq)));

    await Future.wait(jobs);
  }

  void _applyPrediction({required DrivingBehaviorMetrics behavior}) {
    final speedKmh = location.speedKmh;
    ...
    _emergencyStation = _emergencyStation ??
        (_rankedStations.isNotEmpty ? _rankedStations.first.station : null);
  ...
  FuelPredictionState _minimalPrediction() {
    final behavior = _behaviorMetrics ?? _behaviorAnalyzer.snapshot();
    return _predictionEngine.tick(
      fuel: fuel,
      behavior: behavior,
      speedKmh: location.speedKmh,
      routePrediction: null,
      isIdling: (location.speedKmh ?? 0) < 2,
      elevationFactor: 1.0,
      trafficFactor: _trafficFactor,
    );
  }
  ...
  Future<void> _loadNearbyStations({required bool force}) async {
    ...
    try {
      ...
      var result = merged.values.toList(growable: false);
      if (_routePoints.length >= 2) {
        result = filterStationsNearPolyline(
          result,
          _routePoints,
          maxDistanceKm: FuelStationService.routeCorridorMaxKm,
        );
      }
      result.sort((a, b) => a.distanceKm.compareTo(b.distanceKm));
      _nearbyStations = result;
    } catch (_) {
  ...
  Future<void> _loadWeather({bool force = false}) async {
    ...
    try {
      ...
      if (snapshot != null) _weather = snapshot;
    } catch (_) {
  ...
  Future<void> _ensureRoute({
    ...
    _routeLoading = true;
    try {
      final plan = await _routing.planRoute(
        origin: origin,
        destination: dest.location,
      );
      if (seq != _workSeq) return;
      _routePoints = plan.points;
      _routeDistanceKm = plan.distanceKm;
      _routeDurationSeconds = plan.durationSeconds;
